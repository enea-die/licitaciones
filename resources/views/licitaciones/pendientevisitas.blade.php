...
<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
    ...
    <li class="breadcrumb-item text-muted">Licitaciones/Cotizaciones Pendiente Visita</li>
</ul>
@endsection
    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">Listado Licitaciones/Cotizaciones Pendiente Visita</h3>
            </div>
        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(Auth::user()->can('gestionar-visita-licitacion'))
                                                    <button class="btn btn-primary btn-sm" onclick="modalPendienteVisita({{ $row->id }})"">Gestionar visita</button>
                                                @endif
                                            </td>
                                        </tr>
                                <?php }
...
<script>
    ...
    function statusVisita(id){
        if(id === 1){
            //con visita
            $("#divconvisita").css('display','block');
        } else if(id === 2){
            //sin visita
            $("#divconvisita").css('display','none');
        }else{
            $("#divconvisita").css('display','none');
        }
    }
